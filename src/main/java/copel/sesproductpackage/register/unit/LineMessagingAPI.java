...
public class LineMessagingAPI {
     ...
     */
    private static final String DONLOAD_FILE_API_ENDPOINT = "https://api-data.line.me/v2/bot/message/%s/content";
        ...
        try {
            ...
            for (final String message : this.messageList) {
                ...
                if (response.statusCode() != 200) {
                    System.out.println("[WARN] " + response.body());
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public byte[] getFile(final String messageId) throws IOException, InterruptedException {
        ...
        if (response.statusCode() == 200) {
            ...
            System.err.println("[WARN] ファイル取得失敗: HTTP " + response.statusCode());
            return null;
        }
    }
